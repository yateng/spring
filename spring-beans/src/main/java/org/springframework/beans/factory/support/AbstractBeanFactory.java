...
/**
 * Abstract base class for {@link org.springframework.beans.factory.BeanFactory} implementations, providing the full capabilities of the
 ...
 * singleton/prototype determination, {@link org.springframework.beans.factory.FactoryBean} handling, aliases, bean definition merging for child bean
 * definitions, and bean destruction ({@link org.springframework.beans.factory.DisposableBean} interface, custom destroy methods). Furthermore, it can manage a
 * bean factory hierarchy (delegating to the parent in case of an unknown bean), through implementing the
 * {@link org.springframework.beans.factory.HierarchicalBeanFactory} interface.
 * 
 * <p>
 * The main template methods to be implemented by subclasses are {@link #getBeanDefinition} and {@link #createBean}, retrieving a bean definition for a given
 * bean name and creating a bean instance for a given bean definition, respectively. Default implementations of those operations can be found in
...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
     ...
     * 
     * 根据Bean定义的模式，采取的不同创建Bean实例对象的策略，
     * 具体的Bean实例对象的创建过程由实现了ObejctFactory接口的匿名内部类的createBean方法完成，ObejctFactory使用委派模式，
    ...
    protected <T> T doGetBean(final String name, final Class<T> requiredType, final Object[] args, boolean typeCheckOnly) throws BeansException {
        ...
        if (sharedInstance != null && args == null) {
            ...
            // 首先检查是否 能在当前的BeanFactory中获取的所需要的Bean，如果不能则委托当前容器 的父级容器去查找，
            ...
            if (!typeCheckOnly) {
                // 标记Bean已经创建，其实是在一个map中添加标识的Set里加入beanName
            ...
            try {
                ...
                // 要创建的Bean既不是单态模式，也不是原型模式，则根据Bean定义资源中 配置的生命周期范围，选择实例化Bean的合适方法。
    ...
    public boolean isTypeMatch(String name, Class<?> targetType) throws NoSuchBeanDefinitionException {
        ...
        if (beanInstance != null) {
            ...
            return false;// null instance registered
        } else {
            // No singleton instance found -> check bean definition.
            BeanFactory parentBeanFactory = getParentBeanFactory();
            if (parentBeanFactory != null && !containsBeanDefinition(beanName)) {
     ...
     * <p>
     * To be called for BeanWrappers that will create and populate bean instances, and for SimpleTypeConverter used for constructor argument and factory method
     * type conversion.
     * 
     * @param registry
    ...
    protected RootBeanDefinition getMergedBeanDefinition(String beanName, BeanDefinition bd, BeanDefinition containingBd) throws BeanDefinitionStoreException {
        ...
        synchronized (this.mergedBeanDefinitions) {
            ...
            if (mbd == null) {
                    ...
                    try {
                        ...
                        if (!beanName.equals(parentBeanName)) {
                            ...
                            if (getParentBeanFactory() instanceof ConfigurableBeanFactory) {
                                ...
                                throw new NoSuchBeanDefinitionException(bd.getParentName(), "Parent name '" + bd.getParentName() + "' is equal to bean name '"
                ...
                // instance of the bean, or at least have already created an instance before.|Eligible:资格
     ...
     * <p>
     * This implementation is simplistic in that it is not able to handle factory methods and InstantiationAwareBeanPostProcessors. It only predicts the bean
     * type correctly for a standard bean. To be overridden in subclasses, applying more sophisticated type detection.
     * 
     * @param beanName
     *            the name of the bean
     * @param mbd
     ...
     * <p>
     * The default implementation creates the FactoryBean via {@code getBean} to call its {@code getObjectType} method. Subclasses are encouraged to optimize
     * this, typically by just instantiating the FactoryBean but not populating it yet, trying whether its {@code getObjectType} method already returns a type.
     ...
     * <p>
     * Depending on the nature of the concrete bean factory implementation, this operation might be expensive (for example, because of directory lookups in
     * external registries). However, for listable bean factories, this usually just amounts to a local hash lookup: The operation is therefore part of the
     * public interface there. The same implementation can serve for both this template method and the public interface method in that case.
     * 
     * @param beanName
     ...
     * <p>
     * All the other methods in this class invoke this method, although beans may be cached after being instantiated by this method. All bean instantiation
     * within this class is performed by this method.
     * 
     * @param beanName
     *            the name of the bean
     * @param mbd
