...
public class QualifierAnnotationAutowireCandidateResolver implements AutowireCandidateResolver, BeanFactoryAware {
     ...
     * <p>
     * This identifies qualifier annotations for direct use (on fields, method parameters and constructor parameters) as well as meta annotations that in turn
     * identify actual qualifier annotations.
     * <p>
     * This implementation only supports annotations as qualifier types. The default is Spring's {@link Qualifier} annotation which serves as a qualifier for
     * direct use and also as a meta annotation.
     * 
     * @param qualifierType
     ...
     * <p>
     * This setter property exists so that developers can provide their own (non-Spring-specific) annotation type to indicate a default value expression for a
     * specific argument.
     */
     ...
     * <p>
     * To be considered a candidate the bean's <em>autowire-candidate</em> attribute must not have been set to 'false'. Also, if an annotation on the field or
     * parameter to be autowired is recognized by this bean factory as a <em>qualifier</em>, the bean must 'match' against the annotation as well as any
     * attributes it may contain. The bean definition must contain the same qualifier or match by meta attributes. A "value" attribute will fallback to match
