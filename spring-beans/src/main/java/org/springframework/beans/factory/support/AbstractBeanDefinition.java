...
/**
 * Base class for concrete, full-fledged {@link org.springframework.beans.factory.config.BeanDefinition} classes,
...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor implements BeanDefinition, Cloneable {
    ...
    private boolean abstractFlag = false; // 该bean是否抽象
    ...
    private String[] dependsOn; // bean依赖于哪些bean(这些bean必须提前初始化)
    ...
    /* factoryBeanName 和 factoryMethodName 用于factorybean，也就是工厂类型的bean */
     ...
     * <li>Will always take {@code abstract}, {@code scope}, {@code lazyInit}, {@code autowireMode}, {@code dependencyCheck}, and {@code dependsOn} from the
     ...
     * <li>Will override {@code factoryBeanName}, {@code factoryMethodName}, {@code initMethodName}, and {@code destroyMethodName} if specified in the given
     * bean definition.
     * </ul>
     */
     ...
     * <p>
     * Default is singleton status, although this is only applied once a bean definition becomes active in the containing factory. A bean definition may
     * eventually inherit its scope from a parent bean definitionFor this reason, the default scope name is empty (empty String), with singleton status being
     * assumed until a resolved scope will be set.
     * 
     * @see #SCOPE_SINGLETON
     * @see #SCOPE_PROTOTYPE
     */
     ...
     * <p>
     * "Singletons" are the commoner type, so the default is "true". Note that as of Spring 2.0, this flag is just an alternative way to specify
     ...
     * <p>
     * Note that dependencies are normally expressed through bean properties or constructor arguments. This property should just be necessary for other kinds of
     ...
     * for the case of externalized metadata pointing to those. The default is {@code true}; switch this to {@code false} for public access only.
     * <p>
     * This applies to constructor resolution, factory method resolution, and also init/destroy methods. Bean property accessors have to be public in any case
     * and are not affected by this setting.
     * <p>
     * Note that annotation-driven configuration will still access non-public members as far as they have been annotated. This setting applies to externalized
     * metadata in this bean definition only.
     */
    ...
    protected void prepareMethodOverride(MethodOverride mo) throws BeanDefinitionValidationException {
        ...
        if (count == 0) {
            throw new BeanDefinitionValidationException("Invalid method override: no method with name '" + mo.getMethodName() + "' on class ["
                    + getBeanClassName() + "]");
        } else if (count == 1) {
