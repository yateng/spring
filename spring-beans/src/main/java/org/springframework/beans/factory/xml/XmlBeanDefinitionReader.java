 ...
 * 
 * <p>
 * This class loads a DOM document and applies the BeanDefinitionDocumentReader to it. The document reader will register each bean definition with the given
 * bean factory, talking to the latter's implementation of the {@link org.springframework.beans.factory.support.BeanDefinitionRegistry} interface.
 * 
...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
     ...
     * <p>
     * Note that this only activates or deactivates validation itself. If you are switching validation off for schema files, you might need to activate schema
     ...
     * Default is "false".
     * <p>
     * This is typically not needed when schema validation is active. However, without validation, this has to be switched to "true" in order to properly
     ...
     * <p>
     * The default implementation is EmptyReaderEventListener which discards every event notification. External tools can provide an alternative implementation
     ...
     * <p>
     * The default implementation is {@link NullSourceExtractor} which simply returns {@code null} as the source object. This means that - during normal runtime
     * execution - no additional source metadata is attached to the bean configuration metadata.
     */
     ...
     * <p>
     * By default, {@link ResourceEntityResolver} will be used. Can be overridden for custom entity resolution, for example relative to some specific base path.
     */
     ...
     * <p>
     * If not set, a default SimpleSaxErrorHandler is used that simply logs warnings using the logger instance of the view class, and rethrows errors to
    ...
    protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource) throws BeanDefinitionStoreException {
        try {
            ...
            throw new XmlBeanDefinitionStoreException(resource.getDescription(), "Line " + ex.getLineNumber() + " in XML document from " + resource
                    + " is invalid", ex);
     ...
     * validation mode has been configured then the validation mode is {@link #detectValidationMode detected}.
     * <p>
     ...
     * by the supplied {@link Resource}. If the file has a {@code DOCTYPE} definition then DTD validation is used otherwise XSD validation is assumed.
     * <p>
