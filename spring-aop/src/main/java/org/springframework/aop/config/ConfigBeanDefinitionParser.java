...
class ConfigBeanDefinitionParser implements BeanDefinitionParser {
    ...
    /**
     * Configures the auto proxy creator needed to support the {@link BeanDefinition BeanDefinitions} created by the '{@code &lt;aop:config/&gt;}' tag. Will
     * force class proxying if the
    ...
    /**
     * Parses the supplied {@code &lt;advisor&gt;} element and registers the resulting {@link org.springframework.aop.Advisor} and any resulting
     * {@link org.springframework.aop.Pointcut} with the supplied {@link BeanDefinitionRegistry}.
     */
                ...
                if (isAdviceNode(node, parserContext)) {
                    if (!adviceFoundAlready) {
                        ...
                        if (!StringUtils.hasText(aspectName)) {
                            parserContext.getReaderContext().error("<aspect> tag needs aspect bean reference via 'ref' attribute when declaring advices.",
                                    aspectElement, this.parseState.snapshot());
                            return;
                        }
                        beanReferences.add(new RuntimeBeanReference(aspectName));
                    }
                    AbstractBeanDefinition advisorDefinition = parseAdvice(aspectName, i, aspectElement, (Element) node, parserContext, beanDefinitions,
                            beanReferences);
                    beanDefinitions.add(advisorDefinition);
                }
            }

            AspectComponentDefinition aspectComponentDefinition = createAspectComponentDefinition(aspectElement, aspectId, beanDefinitions, beanReferences,
                    parserContext);
    ...
    private AspectComponentDefinition createAspectComponentDefinition(Element aspectElement, String aspectId, List<BeanDefinition> beanDefs,
    ...
    private boolean isAdviceNode(Node aNode, ParserContext parserContext) {
        if (!(aNode instanceof Element)) {
            ...
            return (BEFORE.equals(name) || AFTER.equals(name) || AFTER_RETURNING_ELEMENT.equals(name) || AFTER_THROWING_ELEMENT.equals(name)
                    || AROUND.equals(name));
        }
    }
    ...
    /**
     * Parses the {@code pointcut} or {@code pointcut-ref} attributes of the supplied {@link Element} and add a {@code pointcut} property as appropriate.
     * Generates a {@link org.springframework.beans.factory.config.BeanDefinition} for the pointcut if necessary
