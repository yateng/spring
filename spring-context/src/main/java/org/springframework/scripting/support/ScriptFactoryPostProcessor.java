 ...
 * 
 * <p>
 * This is similar to the {@link org.springframework.beans.factory.FactoryBean} mechanism, but is specifically tailored for scripts and not built into Spring's
 * core container itself but rather implemented as an extension.
 * 
 * <p>
 * <b>NOTE:</b> The most important characteristic of this post-processor is that constructor arguments are applied to the
 * {@link org.springframework.scripting.ScriptFactory} instance while bean property values are applied to the generated scripted object. Typically, constructor
 * arguments include a script source locator and potentially script interfaces, while bean property values include references and config values to inject into
 * the scripted object itself.
 * 
 * <p>
 * The following {@link ScriptFactoryPostProcessor} will automatically be applied to the two {@link org.springframework.scripting.ScriptFactory} definitions
 * below. At runtime, the actual scripted objects will be exposed for "bshMessenger" and "groovyMessenger", rather than the
 * {@link org.springframework.scripting.ScriptFactory} instances. Both of those are supposed to be castable to the example's {@code Messenger} interfaces here.
 * 
 * <pre class="code">
 ...
 * 
 * <p>
 * <b>NOTE:</b> Please note that the above excerpt from a Spring XML bean definition file uses just the &lt;bean/&gt;-style syntax (in an effort to illustrate
 * using the {@link ScriptFactoryPostProcessor} itself). In reality, you would never create a &lt;bean/&gt; definition for a {@link ScriptFactoryPostProcessor}
 * explicitly; rather you would import the tags from the {@code 'lang'} namespace and simply create scripted beans using the tags in that namespace... as part
 * of doing so, a {@link ScriptFactoryPostProcessor} will implicitly be created for you.
 * 
 * <p>
 * The Spring reference documentation contains numerous examples of using tags in the {@code 'lang'} namespace; by way of an example, find below a Groovy-backed
 * bean defined using the {@code 'lang:groovy'} tag.
 * 
 * <pre class="code">
 ...
 * @since 2.0
 */
public class ScriptFactoryPostProcessor extends InstantiationAwareBeanPostProcessorAdapter implements BeanClassLoaderAware, BeanFactoryAware,
        ResourceLoaderAware, DisposableBean, Ordered {
     ...
     * <p>
     * Note that an actual refresh will only happen when the {@link org.springframework.scripting.ScriptSource} indicates that it has been modified.
     * 
    ...
    public Object postProcessBeforeInstantiation(Class<?> beanClass, String beanName) {
        ...
        if (refreshCheckDelay >= 0) {
            Class<?>[] interfaces = scriptFactory.getScriptInterfaces();
            RefreshableScriptTargetSource ts = new RefreshableScriptTargetSource(this.scriptBeanFactory, scriptedObjectBeanName, scriptFactory, scriptSource,
                    isFactoryBean);
     ...
     * If the {@link BeanDefinition} has a {@link org.springframework.core.AttributeAccessor metadata attribute} under the key
     * {@link #REFRESH_CHECK_DELAY_ATTRIBUTE} which is a valid {@link Number} type, then this value is used. Otherwise, the the
    ...
    protected long resolveRefreshCheckDelay(BeanDefinition beanDefinition) {
        ...
        if (attributeValue instanceof Number) {
            ...
            throw new BeanDefinitionStoreException("Invalid refresh check delay attribute [" + REFRESH_CHECK_DELAY_ATTRIBUTE + "] with value '"
    ...
    protected boolean resolveProxyTargetClass(BeanDefinition beanDefinition) {
        ...
        if (attributeValue instanceof Boolean) {
            ...
            throw new BeanDefinitionStoreException("Invalid proxy target class attribute [" + PROXY_TARGET_CLASS_ATTRIBUTE + "] with value '" + attributeValue
     ...
     * <p>
     * By default, supported locators are Spring resource locations (such as "file:C:/myScript.bsh" or "classpath:myPackage/myScript.bsh") and inline scripts
     * ("inline:myScriptText...").
     * 
     * @param beanName
     *            the name of the scripted bean
    ...
    protected Class<?> createConfigInterface(BeanDefinition bd, Class<?>[] interfaces) {
        ...
        for (PropertyValue pv : pvs) {
            ...
            Signature signature = new Signature(setterName, null);
            maker.add(signature, null);
        }
        if (bd instanceof AbstractBeanDefinition) {
            ...
            if (abd.getInitMethodName() != null) {
                ...
                // maker.add(signature, new Type[0]);
                Signature signature = new Signature(abd.getInitMethodName(), null);
                maker.add(signature, null);
            }
     ...
     */
    protected BeanDefinition createScriptedObjectBeanDefinition(BeanDefinition bd, String scriptFactoryBeanName, ScriptSource scriptSource,
            Class<?>[] interfaces) {
     ...
     * @param interfaces
     *            the proxy interfaces (may be {@code null} to
