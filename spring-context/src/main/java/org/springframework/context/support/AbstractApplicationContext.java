...
/**
 * Abstract implementation of the {@link org.springframework.context.ApplicationContext} interface. Doesn't mandate the type of storage used for configuration;
 * simply
 * implements common context functionality. Uses the Template Method design pattern,
 * requiring concrete subclasses to implement abstract methods.
 * 
 * <p>
 * In contrast to a plain BeanFactory, an ApplicationContext is supposed to detect special beans defined in its internal bean factory: Therefore, this class
 * automatically registers {@link org.springframework.beans.factory.config.BeanFactoryPostProcessor BeanFactoryPostProcessors},
 * {@link org.springframework.beans.factory.config.BeanPostProcessor BeanPostProcessors} and {@link org.springframework.context.ApplicationListener
 * ApplicationListeners} which are defined as beans in the context.
 * 
 * <p>
 * A {@link org.springframework.context.MessageSource} may also be supplied as a bean in the context, with the name "messageSource"; otherwise, message
 * resolution is delegated to the parent context. Furthermore, a multicaster for application events can be supplied as "applicationEventMulticaster" bean of
 * type {@link org.springframework.context.event.ApplicationEventMulticaster} in the context; otherwise, a default multicaster of type
 * {@link org.springframework.context.event.SimpleApplicationEventMulticaster} will be used.
 * 
 * <p>
 * Implements resource loading through extending {@link org.springframework.core.io.DefaultResourceLoader}. Consequently treats non-URL resource paths as class
 * path resources (supporting full class path resource names that include the package path, e.g. "mypackage/myresource.dat"), unless the
...
public abstract class AbstractApplicationContext extends DefaultResourceLoader implements ConfigurableApplicationContext, DisposableBean {
     ...
     * {@inheritDoc}
     * <p>
     * If {@code null}, a new environment will be initialized via {@link #createEnvironment()}.
     */
     ...
     * {@link #getEnvironment()} should also be considered. In either case, such modifications should be performed <em>before</em> {@link #refresh()}.
     * 
     ...
     * <p>
     * Note: Listeners get initialized after the MessageSource, to be able to access it within listener implementations. Thus, MessageSource implementations
     ...
     * into Resource instances. Default is a {@link org.springframework.core.io.support.PathMatchingResourcePatternResolver},
     * supporting Ant-style location patterns.
     * <p>
     * Can be overridden in subclasses, for extended resolution strategies, for example in a web environment.
     * <p>
     * <b>Do not call this when needing to resolve a location pattern.</b> Call the context's {@code getResources} method instead, which will delegate to the
     ...
     * {@inheritDoc}
     * <p>
     * The parent {@linkplain ApplicationContext#getEnvironment() environment} is {@linkplain ConfigurableEnvironment#merge(ConfigurableEnvironment) merged}
     * with this (child) application context environment if the parent is non-{@code null} and its environment is an instance of {@link ConfigurableEnvironment}
     * .
     * 
    ...
    public void refresh() throws BeansException, IllegalStateException {
        synchronized (this.startupShutdownMonitor) {
            ...
            // 启动了Bean定义资源的载入、注册过程。obtain:vt.获得，得到;
            ...
            try {
                ...
                // 初始化单例的Bean，并将其保存起来。这里是对容器non-lazy-init属性进行处理的入口方法
    ...
    /**
     * 获得容器ApplicationContext的子类BeanFactory。步骤如下：如果已经有了BeanFactory就销毁它里面的单例Bean并关闭这个BeanFactory。
     * 2.创建一个新的BeanFactory。
     * 3.对这个BeanFactory进行定制（customize),如allowBeanDefinitionOverriding，allowCircularReferences。
    ...
    protected void initApplicationEventMulticaster() {
        ...
        if (beanFactory.containsLocalBean(APPLICATION_EVENT_MULTICASTER_BEAN_NAME)) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("Unable to locate ApplicationEventMulticaster with name '" + APPLICATION_EVENT_MULTICASTER_BEAN_NAME + "': using default ["
     ...
     * <p>
     * Note: This method only works within an active application context, i.e. when an ApplicationEventMulticaster is already available. Generally prefer the
     * use of {@link #addApplicationListener} which is more flexible.
     * 
     * @param listener
    ...
    /**
     * 对配置了non-lazy-init属性的Bean进行预实例化处理
     */
    protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
        ...
        // 对配置了(non-lazy-init)属性的单态模式Bean进行预实例化处理
     ...
     * <p>
     * Delegates to {@code doClose()} for the actual closing procedure. Also removes a JVM shutdown hook, if registered, as it's not needed anymore.
     * 
     * @see #doClose()
     ...
     * "destroy-method".
     * <p>
     * Can be overridden to add context-specific bean destruction steps right before or right after standard singleton destruction, while the context's
     ...
     * <p>
     * Called at the end of {@link #doClose}'s shutdown procedure, after this context's BeanFactory has been closed. If custom shutdown logic needs to execute
     * while the BeanFactory is still active, override the {@link #destroyBeans()} method instead.
     */
     ...
     * <p>
     * A subclass will either create a new bean factory and hold a reference to it, or return a single BeanFactory instance that it holds. In the latter case,
     * it will usually throw an IllegalStateException if refreshing the context more than once.
     * 
     * @throws BeansException
     ...
     * <p>
     * Note: Subclasses should check whether the context is still active before returning the internal bean factory. The internal factory should generally be
    ...
    private class BeanPostProcessorChecker implements BeanPostProcessor {
        ...
        public Object postProcessAfterInitialization(Object bean, String beanName) {
            if (bean != null && !(bean instanceof BeanPostProcessor) && this.beanFactory.getBeanPostProcessorCount() < this.beanPostProcessorTargetCount) {
                if (logger.isInfoEnabled()) {
                    logger.info("Bean '" + beanName + "' of type [" + bean.getClass() + "] is not eligible for getting processed by all BeanPostProcessors "
