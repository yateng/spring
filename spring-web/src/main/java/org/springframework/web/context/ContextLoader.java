 ...
 * 
 * <p>
 * Looks for a {@link #CONTEXT_CLASS_PARAM "contextClass"} parameter at the {@code web.xml} context-param level to specify the context class type, falling back
 * to the default of {@link org.springframework.web.context.support.XmlWebApplicationContext} if not found. With the default ContextLoader implementation, any
 * context class specified needs to implement the ConfigurableWebApplicationContext interface.
 * 
 * <p>
 * Processes a {@link #CONFIG_LOCATION_PARAM "contextConfigLocation"} context-param and passes its value to the context instance, parsing it into potentially
 * multiple file paths which can be separated by any number of commas and spaces, e.g. "WEB-INF/applicationContext1.xml, WEB-INF/applicationContext2.xml".
 * Ant-style path patterns are supported as well, e.g. "WEB-INF/*Context.xml,WEB-INF/spring*.xml" or "WEB-INF/&#42;&#42;/*Context.xml". If not explicitly
 * specified, the context implementation is supposed to use a default location (with XmlWebApplicationContext: "/WEB-INF/applicationContext.xml").
 * 
 * <p>
 * Note: In case of multiple config locations, later bean definitions will override ones defined in previously loaded files, at least when using one of Spring's
 * default ApplicationContext implementations. This can be leveraged to deliberately override certain bean definitions via an extra XML file.
 * 
 * <p>
 * Above and beyond loading the root application context, this class can optionally load or obtain and hook up a shared parent context to the root application
 * context. See the {@link #loadParentContext(ServletContext)} method for more information.
 * 
 * <p>
 ...
 * constructor, allowing for programmatic configuration in Servlet 3.0+ environments. See {@link org.springframework.web.WebApplicationInitializer} for usage
 * examples.
 * 
...
public class ContextLoader {
     ...
     * Specifies the 'selector' used in the {@link ContextSingletonBeanFactoryLocator#getInstance(String selector)} method call, which is used to obtain the
     * BeanFactoryLocator instance from
     * which the parent context is obtained.
     * <p>
     * The default is {@code classpath*:beanRefContext.xml}, matching the default applied for the {@link ContextSingletonBeanFactoryLocator#getInstance()}
     * method. Supplying the "parentContextKey" parameter is sufficient in this case.
     */
     ...
     * used only when obtaining a parent context using the default implementation
     * of {@link #loadParentContext(ServletContext servletContext)}.
     * Specifies the 'factoryKey' used in the {@link BeanFactoryLocator#useBeanFactory(String factoryKey)} method call,
     * obtaining the parent application context from the BeanFactoryLocator instance.
     * <p>
     * Supplying this "parentContextKey" parameter is sufficient when relying on the default {@code classpath*:beanRefContext.xml} selector for candidate
     * factory references.
     */
     ...
     * ContextLoaderListener} subclass as a {@code <listener>} within {@code web.xml}, as a no-arg constructor is required.
     * <p>
     * The created application context will be registered into the ServletContext under the attribute name
     * {@link WebApplicationContext#ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE} and subclasses are free to call the {@link #closeWebApplicationContext} method on
     ...
     * <p>
     * The context may or may not yet be {@linkplain ConfigurableApplicationContext#refresh() refreshed}. If it (a) is an implementation of
     * {@link ConfigurableWebApplicationContext} and (b) has <strong>not</strong> already been refreshed (the recommended approach), then the following will
     * occur:
     * <ul>
     ...
     * <li>Any {@link ApplicationContextInitializer}s specified through the "contextInitializerClasses" init-param will be applied.</li>
     ...
     * determines} what (if any) context initializer classes have been specified through {@linkplain #CONTEXT_INITIALIZER_CLASSES_PARAM context init parameters}
     * and {@linkplain ApplicationContextInitializer#initialize invokes each} with the given web application context.
     * <p>
     * Any {@code ApplicationContextInitializers} implementing {@link org.springframework.core.Ordered Ordered} or marked with @
     * {@link org.springframework.core.annotation.Order Order} will be sorted appropriately.
     * 
     * @param sc
            ...
            if (initializerContextClass != null) {
                Assert.isAssignable(initializerContextClass, wac.getClass(), String.format(
                        "Could not add context initializer [%s] since its generic parameter [%s] "
                                + "is not assignable from the type of application context used by this " + "context loader [%s]: ", initializerClass.getName(),
     ...
     * <p>
     * The main reason to load a parent context here is to allow multiple root web application contexts to all be children of a shared EAR context, or
     * alternately to also share the same parent context that is visible to EJBs. For pure web applications, there is usually no need to worry about having a
     ...
     * {@link #LOCATOR_FACTORY_SELECTOR_PARAM} and {@link #LOCATOR_FACTORY_KEY_PARAM}, to load a parent context which will be shared by all other users of
     * ContextsingletonBeanFactoryLocator which also use the same configuration parameters.
     * 
     * @param servletContext
     *            current servlet context
